              ...
              'text/event-stream': {
                schema: {
                  ...
                  example: [
                    ...
                    'data: {"step":"completed","progress":100,"processing_id":"uuid","result_url":"https://...","result_url_expires_at":"ISO","credits_used":2,"credits_remaining":48,"processing_time_ms":4200}',
                  ].join('\n'),
                },
              },
            },
          },
          '400': { description: 'Missing or invalid fields', content: { 'application/json': { schema: { $ref: '#/components/schemas/Error' }, example: { error: 'Missing required field: person_image' } } } },
          '401': { description: 'Invalid, revoked, or expired API key', content: { 'application/json': { schema: { $ref: '#/components/schemas/Error' }, example: { error: 'Invalid or revoked API key' } } } },
          '403': { description: 'Insufficient credits', content: { 'application/json': { schema: { $ref: '#/components/schemas/Error' }, example: { error: 'Insufficient credits' } } } },
          '429': { description: 'RPM limit exceeded — includes `Retry-After` header', content: { 'application/json': { schema: { $ref: '#/components/schemas/Error' }, example: { error: 'Rate limit exceeded. Retry after 15 seconds.' } } } },
        },
      },
    },
    '/result/{processingId}': {
      get: {
        ...
        responses: {
          ...
          '401': { description: 'Unauthorized', content: { 'application/json': { schema: { $ref: '#/components/schemas/Error' }, example: { error: 'Invalid or revoked API key' } } } },
          '404': { description: 'Not found or belongs to another customer', content: { 'application/json': { schema: { $ref: '#/components/schemas/Error' }, example: { error: 'Processing job not found' } } } },
          '410': { description: 'Job expired (older than 7 days)', content: { 'application/json': { schema: { $ref: '#/components/schemas/Error' }, example: { error: 'Job has expired and is no longer available' } } } },
        },
      },
    },
  },
